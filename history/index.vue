  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <label class="el-form-item-label">回调方式</label>
        <el-input v-model="query.callType" clearable placeholder="回调方式" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        ...
        <label class="el-form-item-label">应用名</label>
        <el-input v-model="query.app" clearable placeholder="应用名" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <date-range-picker
          v-model="query.triggerTime"
          start-placeholder="triggerTimeStart"
          end-placeholder="triggerTimeStart"
          class="date-item"
        />
        <date-range-picker
        ...
        <date-range-picker
          ...
          start-placeholder="modifyTimeStart"
          end-placeholder="modifyTimeStart"
          class="date-item"
        />
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="callType" label="回调方式">
        ...
        <el-table-column prop="runRetry" label="run重试" />
        <el-table-column v-if="checkPer(['admin','historyJob:edit','historyJob:del'])" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudHistoryJob from '@/api/historyJob'
...
const defaultForm = { id: null, triggerTime: null, callName: null, callData: null, callType: null, callHost: null, cron: null, createTime: null, modifyTime: null, app: null, callerrorRetryCount: null, runRetry: null }
export default {
  name: 'HistoryJob',
  ...
  dicts: ['call_type'],
  cruds() {
    return CRUD({ title: 'api/history', url: 'api/historyJob', idField: 'id', sort: 'id,desc', crudMethod: { ...crudHistoryJob }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'historyJob:add'],
        edit: ['admin', 'historyJob:edit'],
        del: ['admin', 'historyJob:del']
      },
      rules: {
        triggerTime: [
          { required: true, message: '调度时间不能为空', trigger: 'blur' }
        ],
        callName: [
          { required: true, message: '回调名称不能为空', trigger: 'blur' }
        ],
        callType: [
          { required: true, message: '回调方式不能为空', trigger: 'blur' }
        ],
        cron: [
